...
@section('content')
        <div class="max-w-xl mx-auto">
            <h1 class="text-xl font-semibold mb-4">Edit Varietas</h1>
            <form action="{{ route('varietas.update', $variety) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <select name="plant_name_id" id="plantSelect" class="w-full border rounded px-3 py-2">
                        ...
                        @foreach ($plants as $plant)
                            <option value="{{ $plant->id }}" @selected($variety->plant_name_id == $plant->id)>{{ $plant->name }}</option>
                        @endforeach
                    </select>
                    @error('plant_name_id')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $variety->name) }}" class="w-full border rounded px-3 py-2">
                    @error('name')
                        <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <script>
            ...
            async function loadPlants(categoryId, selectedId) {
            ...
            if (categorySelect) {
                categorySelect.addEventListener('change', (e) => {
                    loadPlants(e.target.value, null);
                });
            }
        </script>
@endsection
